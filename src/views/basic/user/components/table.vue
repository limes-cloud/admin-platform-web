  ...
  <a-space direction="vertical" fill>
    ...
    >
      ...
      <template #gender="{ record }">
        ...
        <a-tag v-if="!record.gender" color="red">
      ...
      <template #status="{ record }">
        <a-switch
          ...
          :disabled="!$hasPermission('basic:user:status')"
          type="round"
      ...
      <template #operations="{ record }">
        <a-space class="cursor-pointer">
          <a-tag
            v-if="record.id != 1"
            v-permission="'basic:user:reset:password'"
            color="orangered"
          ...
          <template v-if="$hasPermission('basic:user:reset:password')">
            <a-popconfirm
              v-if="record.id != 1"
              content="您确认重置此用户密码？"
              type="warning"
              @ok="emit('resetPassword', record.id)"
            >
              <a-tag color="red">
                <template #icon><icon-refresh /></template> 重置密码
              </a-tag>
            </a-popconfirm>
          </template>

          <template v-if="$hasPermission('basic:user:offline')">
            <a-popconfirm
              v-if="record.id != 1"
              content="您确认要下线此用户"
              type="warning"
              @ok="emit('offline', record.id)"
            >
              <a-tag color="red">
                <template #icon><icon-poweroff /></template> 下线
              </a-tag>
            </a-popconfirm>
          </template>

          <template v-if="$hasPermission('basic:user:delete')">
            <a-popconfirm
              v-if="record.id != 1"
              content="您确认删除此用户"
              type="warning"
