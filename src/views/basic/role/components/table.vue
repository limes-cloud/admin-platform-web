<template>
  ...
  >
    ...
    <template v-if="$hasPermission('basic:role:menu')" #menu="{ record }">
      <a-tag
        v-if="record.id != 1"
        class="cursor-pointer"
        color="arcoblue"
        @click="emit('updateMenu', record)"
      >
    ...
    <template #operations="{ record }">
      <a-space class="cursor-pointer">
        ...
        <!--- 自定义指令无法绑定到组建上 -->
        <template v-if="$hasPermission('basic:role:delete')">
          <a-popconfirm
            v-if="record.id != 1"
            content="您确认删除此菜单"
            type="warning"
